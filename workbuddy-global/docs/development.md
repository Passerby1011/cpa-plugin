...
# Or explicitly
CGO_ENABLED=1 go build -buildmode=c-shared -ldflags "-X main.version=$(git describe --tags --always)" -o workbuddy-global.so .
```

Output: `workbuddy-global.so` (and `workbuddy-global.h` as a side effect).
...
# 4. Tail logs
docker logs -f cpa-manager-plus-cli-proxy-api-1 | grep -i workbuddy-global
```
...
# Panel API
curl http://localhost:8317/v0/management/plugins/workbuddy-global/accounts \
