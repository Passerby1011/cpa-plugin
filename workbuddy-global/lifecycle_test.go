...
func TestSafeWorkbuddyAuthPath(t *testing.T) {
	dir := t.TempDir()
	ok := filepath.Join(dir, "workbuddy-global-abc.json")
...
func TestDeleteAuthFile_Idempotent(t *testing.T) {
	dir := t.TempDir()
	p := filepath.Join(dir, "workbuddy-global-x.json")
	if err := os.WriteFile(p, []byte(`{"type":"workbuddy-global"}`), 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestAuthFileNameFor(t *testing.T) {
	...
	if got := authFileNameFor(sa); got != "workbuddy-global-uid-1.json" {
		t.Fatalf("got %q", got)
	}
...
func TestIsSafeWorkbuddyAuthPath(t *testing.T) {
	dir := t.TempDir()
	ok := filepath.Join(dir, "workbuddy-global-safe.json")
	if !isSafeWorkbuddyAuthPath(ok) {
		t.Fatalf("want safe: %s", ok)
	}
	legacy := filepath.Join(dir, "workbuddy-global.json")
	...
	if isSafeWorkbuddyAuthPath("/auth/../workbuddy-global-x.json") {
		t.Fatal("path with .. must be rejected")
	}
	// /etc/workbuddy-global-evil.json has valid basename but is NOT under auth dir.
...
func TestDeleteAuthFileInDir(t *testing.T) {
	dir := t.TempDir()
	target := filepath.Join(dir, "workbuddy-global-x.json")
	...
	if err := deleteAuthFileInDir("workbuddy-global-evil.json", ""); err == nil {
...
func TestListEntryMatchesUID(t *testing.T) {
	...
	want := "workbuddy-global-" + uid + ".json"
	cases := []struct {
		name string
		...
		{"basename id", pluginapi.HostAuthFileEntry{Name: "workbuddy-global-" + uid}, true},
		{"case", pluginapi.HostAuthFileEntry{Name: strings.ToUpper(want)}, true},
		{"other uid", pluginapi.HostAuthFileEntry{Name: "workbuddy-global-other.json"}, false},
		{"legacy bare", pluginapi.HostAuthFileEntry{Name: "workbuddy-global.json"}, false},
