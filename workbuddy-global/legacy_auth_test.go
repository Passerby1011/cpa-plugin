...
func TestIsWorkbuddyAuthListNameIncludesLegacy(t *testing.T) {
	for _, name := range []string{"workbuddy-global.json", "workbuddy-global-u1.json", "WORKBUDDY-U2.JSON"} {
...
func TestShouldDeleteLegacyForUIDRequiresMatchingUID(t *testing.T) {
	...
	if shouldDeleteLegacyForUID(legacyA, "B") {
		t.Fatal("must not delete legacy workbuddy-global.json for a different uid")
	}
	if !shouldDeleteLegacyForUID(legacyA, "A") {
		t.Fatal("should delete legacy workbuddy-global.json when uid matches migrated account")
	}
}
