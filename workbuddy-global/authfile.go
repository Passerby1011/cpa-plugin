...
// workbuddy-global-<uid>.json naming rule, UID sanitization (path-traversal defense),
...
// authFileNameFor matches toAuthData naming: always workbuddy-global-<uid>.json when UID is known.
// Bare "workbuddy-global.json" is legacy single-account only (no UID).
...
// with multi-account workbuddy-global-<uid>.json for the same credential.
...
// Prefer workbuddy-global-<uid>.json; if the host still points at legacy workbuddy-global.json
...
// when the canonical name differs (workbuddy-global.json → workbuddy-global-<uid>.json).
...
func isSafeWorkbuddyAuthPath(path string) bool {
	...
	// Accept both this plugin's own prefix (workbuddy-global-) and the legacy
	// upstream prefix (workbuddy-). An auth store carried over from the
	// combined workbuddy plugin holds workbuddy-<uid>.json / workbuddy.json
	// files; refusing them would silently strand every existing credential.
	if !strings.HasPrefix(lower, "workbuddy-global-") &&
		!strings.HasPrefix(lower, "workbuddy-") &&
		lower != "workbuddy-global.json" && lower != "workbuddy.json" {
		return false
	}
