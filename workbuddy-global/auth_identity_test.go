...
func TestHandleParseAuth_EchoesFileNameAndEmptyID(t *testing.T) {
	...
	req := pluginapi.AuthParseRequest{
		...
		Path:     "/root/.cli-proxy-api/workbuddy-global-" + uid + ".json",
...
func TestHandleParseAuth_LegacyWorkbuddyJSON_KeepsHostFileName(t *testing.T) {
	// Even when storage has a UID, parse must not rename to workbuddy-global-<uid>.json.
	uid := "00e26541-1884-4916-9c26-253a325d64ac"
	req := pluginapi.AuthParseRequest{
		Provider: providerName,
		Path:     "/root/.cli-proxy-api/workbuddy-global.json",
		FileName: "workbuddy-global.json",
		RawJSON:  sampleNestedAuthJSON(uid),
	}
	body, _ := json.Marshal(req)
	out, err := handleParseAuth(body)
	...
	resp := decodeParseAuth(t, out)
	if resp.Auth.FileName != "workbuddy-global.json" {
		t.Fatalf("FileName=%q want workbuddy-global.json (no rename)", resp.Auth.FileName)
	}
	if resp.Auth.ID != "" {
...
func TestToAuthData_LoginImportStillSetsUIDFileName(t *testing.T) {
	// New accounts (login/import) intentionally use workbuddy-global-<uid>.json
	sa := &storedAuth{
	...
	if ad.FileName != "workbuddy-global-abc-uid.json" {
...
func TestToAuthDataForLoginPollKeepsCanonicalFileNameAndClearsID(t *testing.T) {
	...
	if got.FileName != "workbuddy-global-uid-1.json" {
