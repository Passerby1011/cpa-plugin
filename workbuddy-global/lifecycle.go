...
func deleteAuth(authIndex, authID string, sa *storedAuth) error {
	...
	// Also remove legacy workbuddy-global.json if this UID was dual-named historically.
...
// (workbuddy-global-<uid>.json) is deliberately not part of the check: keepalive.go's
...
func logUpstreamClassification(status int, kind upstreamErrKind, authID string) {
	...
	hostLogf("info", fmt.Sprintf("workbuddy-global upstream %d classified as %s (account untouched, auth=%s)",
...
func applyFaultLifecycleEffect(authID string, kind upstreamErrKind, status int, body string) {
	...
	case upstreamErrSessionDead, upstreamErrAccountBanned:
		...
		if err := disableAuth(idx, id, sa, nil, reason); err != nil {
			hostLogf("warn", fmt.Sprintf("workbuddy-global %s on %s: disable failed: %v", kind.String(), shortUID(authID), err))
			return
		}
		hostLogf("warn", fmt.Sprintf("workbuddy-global %s on auth=%s (http %d): disabled until re-login", kind.String(), shortUID(authID), status))
	case upstreamErrAccountNotActivated:
		hostLogf("debug", fmt.Sprintf("workbuddy-global 14017 trial not activated on auth=%s (http %d): soft cooldown, account left enabled",
			shortUID(authID), status))
	case upstreamErrModelRateLimit:
		message := fmt.Sprintf("workbuddy-global 6004 model rate limit on auth=%s (http %d): account healthy, model throttled",
			shortUID(authID), status)
...
func resolveAuthIndexAndID(authID string) (string, string) {
	...
	// Multi-account files are workbuddy-global-<uid>.json; list Name/ID usually carry that.
	wantName := "workbuddy-global-" + authID + ".json"
	for _, f := range files {
...
func invalidateAccountCredits(authID, authUID string) {
	...
	wantName := "workbuddy-global-" + authUID + ".json"
	matchedByName := false
	for _, f := range files {
		if f.AuthIndex == authID || f.ID == authID || f.Name == authID {
...
// (workbuddy-global-<uid>.json naming). Pure helper for O(list) cache invalidation.
func listEntryMatchesUID(f pluginapi.HostAuthFileEntry, uid, wantName string) bool {
	...
	return strings.EqualFold(base, "workbuddy-global-"+uid)
}
