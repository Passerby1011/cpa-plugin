...
func TestOriginRefererFor(t *testing.T) {
	cases := []struct {
		...
		{"nil (defaults to plugin realm Global)", nil, profileFor(RealmGlobal).Origin},
		{"empty domain (CN legacy)", &storedAuth{}, profileFor(RealmCN).Origin},
		{"CN domain", &storedAuth{Auth: storedTokens{Domain: "www.codebuddy.cn"}}, profileFor(RealmCN).Origin},
		{"Global domain", &storedAuth{Auth: storedTokens{Domain: "www.workbuddy.ai"}}, profileFor(RealmGlobal).Origin},
	}
	for _, tc := range cases {
