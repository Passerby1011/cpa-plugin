...
func isWorkbuddyAuthListName(name string) bool {
	...
	// Accept this plugin's own canonical name/prefix and the legacy upstream
	// ones. An auth store carried over from the combined workbuddy plugin holds
	// workbuddy.json / workbuddy-<uid>.json; refusing them would silently strand
	// every existing credential. "workbuddy-global-" is deliberately NOT treated
	// as a match for the bare "workbuddy-" branch's own file, so both spellings
	// are listed explicitly.
	if name == authFileName || strings.HasPrefix(name, providerName+"-") {
		return true
	}
	return name == legacyAuthFileName || strings.HasPrefix(name, legacyAuthPrefix)
}
