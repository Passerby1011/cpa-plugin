...
// workbuddy-global.so binary (symbol table, string constants and RPC shape) published
...
const (
	...
	// auth store keeps working after switching to this plugin.
	legacyAuthFileName = "workbuddy.json"
	legacyAuthPrefix   = "workbuddy-"
...
func parseStored(raw []byte) (*storedAuth, error) {
	...
	//   flat:   {"accessToken":...,"uid":...,"nickname":...} (CPA-Manager-Plus auths/workbuddy-global.json)
...
// commonHeaders applies realm-independent headers plus a default User-Agent.
//
// The default UA keeps the helper safe on its own (it is called from paths
// where no account is resolved yet), while realm-aware callers override it via
// commonHeadersFor / applyOAuthProfileHeaders with the correct platform segment.
func commonHeaders(req *http.Request) {
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Accept", "application/json, text/plain, */*")
	req.Header.Set("X-Requested-With", "XMLHttpRequest")
	if req.Header.Get("User-Agent") == "" {
		req.Header.Set("User-Agent", userAgentFor(RealmCN))
	}
}

// commonHeadersFor applies the full header set for an account's realm.
// Unlike commonHeaders it can set the realm-dependent Origin/Referer/User-Agent,
// so it is preferred wherever an account is available.
func commonHeadersFor(req *http.Request, sa *storedAuth) {
	commonHeaders(req)
	origin := originRefererFor(sa)
	req.Header.Set("Origin", origin)
	req.Header.Set("Referer", origin+"/")
	req.Header.Set("User-Agent", userAgentFor(realmForAuth(sa)))
}

// originRefererFor returns the Origin/Referer base URL appropriate for the
// account's realm. Global accounts use https://www.workbuddy.ai; CN (and
// legacy auth files with empty domain) use https://www.codebuddy.cn.
func originRefererFor(sa *storedAuth) string {
	return profileFor(realmForAuth(sa)).Origin
}
...
func acceptLanguageFor(sa *storedAuth) string {
	return profileFor(realmForAuth(sa)).AcceptLanguage
}
...
func upstreamBaseFor(sa *storedAuth) string {
	return profileFor(realmForAuth(sa)).Base
}
...
func toAuthDataOpts(sa *storedAuth, cr *creditsSummary, disabled bool, carrier map[string]any) pluginapi.AuthData {
	...
	if sa != nil {
		if uid := sanitizeUIDForFileName(sa.Account.UID); uid != "" {
			id = uid
			fileName = "workbuddy-global-" + uid + ".json"
		}
	}
...
func handleExecExecute(raw []byte) ([]byte, error) {
	...
	if statusCode >= 400 {
		...
		if hint := retryAfterHint(respHeaders); hint != "" {
			hostLogf("warn", fmt.Sprintf("workbuddy-global upstream %d on auth=%s: upstream asks to wait %s",
