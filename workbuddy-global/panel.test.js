...
test("query key replaces session key once and is removed from URL", () => {
  ...
  const { storage } = loadPanel({
    sessionEntries: [["workbuddy-global-mgmt-key", "old-value"]],
    location,
  ...
  assert.equal(storage.get("workbuddy-global-mgmt-key"), "secret-value");
...
test("load updates model status before dashboard error", async () => {
  ...
  storage.set("workbuddy-global-mgmt-key", "test-key");
  let toastCalls = 0;
...
test("panel transport errors use a fixed message", async () => {
  ...
  await assert.rejects(context.managementAPI("/plugins/workbuddy-global/config"), error => error.message === "网络请求失败");
...
test("panel response auth failures clear the key without reading bodies", async () => {
  for (const [status, message] of [[401, "management key 无效或缺失"], [403, "禁止访问 (403)"]]) {
    ...
    assert.equal(storage.has("workbuddy-global-mgmt-key"), false);

    assert.equal(context.storeSessionKey("test-key"), true);
    await assert.rejects(context.managementAPI("/plugins/workbuddy-global/config"), error => error.message === message);
    assert.equal(storage.has("workbuddy-global-mgmt-key"), false);
