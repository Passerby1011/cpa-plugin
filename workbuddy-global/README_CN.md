...
把编译好的 `workbuddy-global.so` 放到 CPA 插件目录：
...
`/v0/resource/plugins/workbuddy-global/panel`），点 **登录** 走 OAuth 流程。
每个账号登录一次，插件会把 `workbuddy-global-<uid>.json` 写入 auth store。
...
全部字段可选，位于 `plugins.configs.workbuddy-global` 下。
...
plugins:
  configs:
    workbuddy-global:
      ...
      # 插件层 management 鉴权。设置后所有 /v0/management/plugins/workbuddy-global/*
