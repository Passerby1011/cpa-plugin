...
func oauthProfileForMode(mode string) oauthRequestProfile {
	cnProfile := profileFor(RealmCN)
	globalProfile := profileFor(RealmGlobal)
	switch mode {
	case oauthClientModeWorkBuddy:
		return oauthRequestProfile{
			mode:      oauthClientModeWorkBuddy,
			base:      cnProfile.Base,
			stateURL:  cnProfile.Base + pluginAuthStatePath + "?platform=workbuddy",
			userAgent: userAgentFor(RealmCN),
			origin:    cnProfile.Origin,
		}
	case oauthClientModeWorkBuddyAI:
		return oauthRequestProfile{
			mode:      oauthClientModeWorkBuddyAI,
			base:      globalProfile.Base,
			stateURL:  globalProfile.Base + pluginAuthStatePath + "?platform=workbuddy-ai",
			userAgent: userAgentFor(RealmGlobal),
			origin:    globalProfile.Origin,
		}
	}
	return oauthRequestProfile{
		mode:      oauthClientModeCLI,
		base:      cnProfile.Base,
		stateURL:  cnProfile.Base + pluginAuthStatePath + "?platform=CLI",
		userAgent: userAgentFor(RealmCN),
		origin:    cnProfile.Origin,
	}
}
...
// "workbuddy-global-<uid>.json" while the original file was "workbuddy-global.json"
