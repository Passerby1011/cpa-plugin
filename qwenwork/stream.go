...
func pumpUpstreamStream(httpReq *http.Request, cancel context.CancelFunc, streamID string, sseFramed bool, requestedModel, upstreamModel, authUID string, started time.Time, authID string) {
	...
	for scanner.Scan() {
		...
		bodyStr, errMsg, ok := unwrapQwenSSE(payload)
		if !ok {
			if errMsg != "" {
				publishUsage(requestedModel, upstreamModel, authUID, started, collector.detail(), true, 400, errMsg)
...
// collectUpstreamStreamQoder is the QwenWork-flavoured synchronous fallback
...
func collectUpstreamStreamQoder(encodedBody string, sa *storedAuth, modelKey string, sseFramed bool, collector *sseUsageCollector) ([]pluginapi.ExecutorStreamChunk, int, error) {
	...
	for scanner.Scan() {
		...
		if !ok {
			if errMsg != "" {
				return nil, 400, fmt.Errorf("%s", errMsg)
			}
			continue
		}
		if collector != nil {
...
// (QwenWork emits these on the terminal chunk, and strict clients interpret
...
// aggregateQoderSSE folds QwenWork's nested SSE stream into a single
...
func unwrapQwenSSE(payload string) (string, string, bool) {
	...
	if _, has := outer["choices"]; has {
		return text, "", true
	}
	if obj, _ := outer["object"].(string); obj == "chat.completion.chunk" || obj == "chat.completion" {
		return text, "", true
	}
	// Error envelope: {"headers":...,"body":"<err json>","statusCodeValue":403}
	if sv, ok := outer["statusCodeValue"].(float64); ok && sv >= 400 {
		return "", bodyCodeMessage(fmtBodyText(outer["body"]), int(sv)), false
	}
	inner, present := outer["body"]
	if !present {
		return "", "", false
	}
	switch v := inner.(type) {
	case string:
		if v == "" || v == "[DONE]" || v == "{}" || v == "null" {
			return "", "", false
		}
		if m := bodyCodeMessage(v, 0); m != "" {
			return "", m, false
		}
		return v, "", true
...
// fmtBodyText renders an envelope body (string or structured) as a string.
func fmtBodyText(v any) string {
	switch x := v.(type) {
	case string:
		return x
	case map[string]any, []any:
...
func bodyCodeMessage(raw string, status int) string {
	...
	if json.Unmarshal([]byte(raw), &inner) != nil {
		if status >= 400 {
			return fmt.Sprintf("upstream %d: %s", status, truncateRedacted(raw, 200))
		}
		return ""
	}
	if c, _ := inner["code"].(string); c == "400" || c == "401" || c == "403" {
