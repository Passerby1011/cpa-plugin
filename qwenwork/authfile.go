...
// qwenwork-<uid>.json naming rule, UID sanitization (path-traversal defense),
...
// authFileNameFor matches toAuthData naming: always qwenwork-<uid>.json when UID is known.
// Bare "qwenwork.json" is legacy single-account only (no UID).
...
// with multi-account qwenwork-<uid>.json for the same credential.
...
// Prefer qwenwork-<uid>.json; if the host still points at legacy qwenwork.json
...
// when the canonical name differs (qwenwork.json → qwenwork-<uid>.json).
