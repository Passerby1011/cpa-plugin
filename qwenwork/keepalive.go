...
// QwenWork returns TOKEN_EXPIRE when jt- or jrt- is no longer valid.
...
func refreshOneAuth(authIndex, authID string) (string, error) {
	...
	// deviceToken refresh returns {"device_token":...} (or "token") plus a
	// rotating refresh_token and an RFC3339 expires_at / ms expires_in.
	...
	sa.Auth.AccessToken = dt.accessToken()
	if dt.RefreshToken != "" {
		sa.Auth.RefreshToken = dt.RefreshToken
	}
	sa.Auth.ExpiresAt = preserveExpiry(deviceExpiryUnix(&dt), sa.Auth.ExpiresAt)
	...
	// access token. Without this, the old signature persists and all
