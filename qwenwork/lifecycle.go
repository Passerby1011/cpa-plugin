...
var (
	...
	// authWriteLocks serializes lifecycle writes (disable/reenable/delete/note)
	// per auth_index — prevents concurrent reconcile vs dashboard from racing.
	authWriteLocks sync.Map // auth_index -> *sync.Mutex
)

// authWriteLock returns a per-auth_index mutex for lifecycle writes.
func authWriteLock(authIndex string) *sync.Mutex {
	v, _ := authWriteLocks.LoadOrStore(authIndex, &sync.Mutex{})
	return v.(*sync.Mutex)
}
...
func resolveAuthIndexAndID(authID string) (string, string) {
	...
	// Multi-account files are qwenwork-<uid>.json; list Name/ID usually carry that.
...
// (qwenwork-<uid>.json naming). Pure helper for O(list) cache invalidation.
