...
func TestRefreshMetadataPreservesUserConfiguredFields(t *testing.T) {
	sa := &storedAuth{
		Auth:    storedTokens{AccessToken: "at", RefreshToken: "rt", Domain: "gateway.qwenwork.cn"},
...
func TestParseAuthCarrierKeepsOnlyUserOwnedKeys(t *testing.T) {
	raw := []byte(`{
		"type":"qwenwork","provider":"qwenwork","logo":"l","disabled":false,
		"note":"CN · 免费 0/2000",
...
func TestParseAuthCarrierRoundTripsThroughAuthData(t *testing.T) {
	raw := []byte(`{"type":"qwenwork","note":"n","weight":9,"prefix":"qw","proxy_url":"http://p:2","auth":{"accessToken":"at","refreshToken":"rt"},"account":{"uid":"u1"}}`)
	carrier := parseAuthMetadataCarrier(raw)
...
func TestBuildAuthFileJSONPreservesExistingPhysicalFields(t *testing.T) {
	physical := []byte(`{"type":"qwenwork","prefix":"qw","weight":5,"proxy_url":"socks5://127.0.0.1:1080","auth":{"accessToken":"old_at","refreshToken":"old_rt"},"account":{"uid":"u1"}}`)
	sa := &storedAuth{
