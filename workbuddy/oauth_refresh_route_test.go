...
func TestRefreshFollowsAccountDomainNotLoginChannel(t *testing.T) {
	const desktopUA = "WorkBuddy/5.3.14 WorkBuddy/5.3.14 CLI/2.115.0"
	cases := []struct {
		name       string
		mode       string
		domain     string
		...
		{"global account under CN mode", oauthClientModeWorkBuddy, "www.workbuddy.ai", "www.workbuddy.ai", desktopUA, originRefererGlobal},
		{"global account under intl mode", oauthClientModeWorkBuddyAI, "www.workbuddy.ai", "www.workbuddy.ai", desktopUA, originRefererGlobal},
		{"CN account under CN mode", oauthClientModeWorkBuddy, "www.codebuddy.cn", "copilot.tencent.com", desktopUA, "https://www.workbuddy.cn"},
		{"CN account under intl mode", oauthClientModeWorkBuddyAI, "www.codebuddy.cn", "copilot.tencent.com", desktopUA, "https://www.workbuddy.cn"},
		{"legacy CN account without domain under cli mode", oauthClientModeCLI, "", "copilot.tencent.com", clientUA, originReferer},
	}
...
func TestOAuthProfileForAuthPicksRealm(t *testing.T) {
	cases := []struct {
		...
		{"global account", oauthClientModeCLI, "www.workbuddy.ai", upstreamBaseGlobal},
		{"cn desktop account", oauthClientModeWorkBuddy, "www.codebuddy.cn", upstreamBaseCN},
		{"cn account with intl login channel collapses to cn gateway", oauthClientModeWorkBuddyAI, "www.codebuddy.cn", upstreamBaseCN},
		{"legacy cn account without domain", oauthClientModeWorkBuddy, "", upstreamBaseCN},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			withOAuthClientMode(t, tc.mode)
