...
func TestBackendHeadersSendClientMarkerAndLanguage(t *testing.T) {
	cases := []struct {
		...
		{"CN account", "www.codebuddy.cn", acceptLanguage},
		{"Global account", "www.workbuddy.ai", acceptLanguageGlobal},
		{"empty domain falls back to CN", "", acceptLanguage},
	}
...
func TestBillingHeadersSendClientMarkerAndLanguage(t *testing.T) {
	for _, tc := range []struct {
		...
		{"www.codebuddy.cn", acceptLanguage},
		{"www.workbuddy.ai", acceptLanguageGlobal},
	} {
...
func TestAcceptLanguageForNilAuth(t *testing.T) {
	if got := acceptLanguageFor(nil); got != acceptLanguage {
		t.Errorf("acceptLanguageFor(nil) = %q, want %q", got, acceptLanguage)
	}
}
