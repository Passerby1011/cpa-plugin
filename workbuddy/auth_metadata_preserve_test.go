...
func TestParseAuthCarrierKeepsOnlyUserOwnedKeys(t *testing.T) {
	raw := []byte(`{
		"type":"workbuddy","provider":"workbuddy","logo":"l","disabled":false,
		"note":"CN · 余1 已用2 池3",
...
func TestParseAuthCarrierRoundTripsThroughAuthData(t *testing.T) {
	raw := []byte(`{"type":"workbuddy","note":"n","weight":9,"proxy_url":"http://p:2","auth":{"accessToken":"at","refreshToken":"rt"},"account":{"uid":"u1"}}`)
	carrier := parseAuthMetadataCarrier(raw)
